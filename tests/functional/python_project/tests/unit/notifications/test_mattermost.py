...
class MattermostTest(unittest.TestCase):
    ...
    def test_send_notification(self, mock_get_channel_id,
                               mock_get, mock_post):
        ...
        mock_post.assert_called_with('http://localhost/posts',
                                     headers={
    ...
    @patch('charon.notifications.mattermost.Mattermost._get_channel_id')
    def test_send_notification_failure(self, mock_get_channel_id, 
                                       mock_get, mock_post):
    ...
    def test_post(self, mock_post):
        ...
        mock_post.assert_called_once_with('http://localhost1',
                                          headers={
                                              'Authorization': 'Bearer test'
                                          },
                                          data=2,
    ...
    def test_get_team_id(self, mock_get):
        ...
        mock_get.assert_called_once_with('http://localhost/teams/name/test',
                                          headers={
                                              'Authorization': 'Bearer test'
                                          },
                                          verify=False)
    ...
    def test_get_channel_id(self, mock_get):
        ...
        mock_get.assert_called_once_with('http://localhost/teams/{0}/channels/name/{1}'.format(team_id, channel_name),
                                         headers={
                                             'Authorization': 'Bearer test'
                                         },
                                         verify=False)
